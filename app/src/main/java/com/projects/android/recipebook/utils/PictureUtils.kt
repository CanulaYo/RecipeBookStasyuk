    ...
    companion object {
        ...
        fun getUriForFile(context: Context, pictureFile: File): Uri {
            return FileProvider.getUriForFile(
                context,
                "com.projects.android.recipebook.fileprovider",
                pictureFile
            )
        }
        ...
        fun getPicture(context: Context, pictureName: String): File {
            val imagePath =
                File(context.applicationContext.filesDir, "pictures").also { it.mkdirs() }
        ...
        fun deletePicture(context: Context, pictureName: String) {
            if (getPicture(context, pictureName).delete().not()) {
                ErrorUtil.shortToast(context, "Не удалось удалить фотографию")
            }
        }
        ...
        fun savePicture(context: Context, pictureName: String) {
            ...
            try {
                FileUtils.copy(
                    FileInputStream(getCachedPicture(context, pictureName)),
                    FileOutputStream(pictureFile)
                )

                val outputWidth = 1024
                val outputQuality = 75 //
                ...
                val outputBitmap =
                    Bitmap.createScaledBitmap(bitmap, outputWidth, outputHeight, false)
                ...
                ErrorUtil.shortToast(context, "Не удалось сохранить фотографию")
            }
        }

        private fun rotateImage(context: Context, uri: Uri, bitmap: Bitmap): Bitmap? {
            val input: InputStream? = context.contentResolver.openInputStream(uri)
            val ret = when (ExifInterface(input!!).getAttributeInt(
