...
class ListHolderRecipes(private val binding: ItemListRecipesBinding, private val context: Context) :
    ...
    fun bind(recipe: Recipe, onRecipeClicked: (recipeID: Int) -> Unit) {
        ...
        binding.apply {
            ...
            isVegItemList.setImageDrawable(
                AppCompatResources.getDrawable(
                    context,
                    if (recipe.isVeg) R.drawable.is_veg else R.drawable.is_not_veg
                )
            )
            isCookedItemList.setImageDrawable(
                AppCompatResources.getDrawable(
                    context,
                    if (recipe.isCooked) R.drawable.is_cooked else R.drawable.is_not_cooked
                )
            )
        }
    }
